...
public class Main {
    ...
    private  void outputTest(OutputFactory outputFactory, String test, int k, int N, int[] numbers) {
        try {
    ...
    private void inputTest(InputFactory inputFactory, String test, int k) {
        try {
            int[] dummy = new int[1];
            SpeedTest speedTest = new SpeedTest(dummy);
            StopWatch stopWatch = new Log4JStopWatch(test + " " + k);
...
*//*
        int[] numbers = new Generator(N).generateNumbers();

        for(int j = 1 ; j < K ; j++ ) {
            for(int i = 0 ; i < averageOver ; i++ ) {
                obj.outputTest(o4, "Output 4", j, N, numbers);
                obj.deleteOutputfiles(j);
            }
        }
        for(int j = 1 ; j < K ; j++ ) {
            for(int i = 0 ; i < averageOver ; i++ ) {
...
/*
        ...
        for(int j = 0 ; j < K ; j++ ) {
            for(int i = 0 ; i < averageOver ; i++ ) {
                obj.outputTest(o2, "Output 2", j, N, numbers);
                obj.deleteOutputfiles(j);
        ...
        for(int j = 0 ; j < K ; j++ ) {
            for(int i = 0 ; i < averageOver ; i++ ) {
                obj.outputTest(o1, "Output 1", j, N, numbers);
                obj.deleteOutputfiles(j);
            }
        }
...
/*
        ...
        for(int bufferSize = 2000 ; bufferSize <= maxBufferSize ; bufferSize *= stepBufferSize){
            for(int i = 0 ; i < averageOver ; i++ ){
                o3 = new Output3Factory(bufferSize);
                obj.outputTest(o3, "Output 3 B_"+ bufferSize, K, N,numbers);
